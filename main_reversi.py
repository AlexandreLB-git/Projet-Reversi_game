...
def run_tournament():
    ...
    for name in sorted_agents:
        s = scores[name]
        r = results[name]
    logger.info(f"{name:20} | Score: {s:4.1f} | W: {r['wins']} D: {r['draws']} L: {r['losses']}")
    logger.info("\n*** History \n***")
    for name in sorted_agents:
        moves_history = "|".join(f"{m1} vs {m2} : {score}" for (m1, m2, score) in history[name])
        logger.info(f"{name} : {moves_history}")
